...
int main() {
    ...
    while (t--) {
        ...
        // Now, we also need to check the special case for consecutive numbers
        for (int i = 0; i < uniqueFreq.size(); ++i) {
            if (i + 1 < uniqueFreq.size() && uniqueFreq[i].first + 1 == uniqueFreq[i + 1].first) {
                // We can use the current and next number in the sequence
                int count = uniqueFreq[i].second + uniqueFreq[i + 1].second;
